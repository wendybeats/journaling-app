...
- Rule: **views never reference raw hex or raw point sizes — only semantic tokens.** A view uses `Vellum.text.written`, never `#1A1A1A`.
...
enum Vellum {
    ...
    // e.g. Vellum.text.written → ink (light) / boneType (dark)
}
```
...
- [ ] Values mirrored into a SwiftUI `Vellum` theme file, ready for Stage 1 screens
