# Vellum — Reflection Layer Spec (V2)
...
- **Content — a paged, full-screen sequence in the dot language** (the one place Vellum is allowed a little theater, still monochrome, still typographic):
...
- Surfaced the first time a recap *could* exist (first Sunday meeting the weekly threshold) — an in-system card, not a system dialog:
  ...
  - Body (serif): "Vellum can read your week back to you — your own words, collected. The writing is sent once, privately, to generate it, then discarded. Nothing is stored anywhere but here."
