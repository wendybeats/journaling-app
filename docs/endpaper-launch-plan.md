...
| A8 | **Calendar — dynamic register** | ◐ Built July 27 per the gameplan: <1 month of entries lands on one big month; 2+ months get the year breakdown; multi-year data stacks year sections. Year mini-matrix morphs into the month grid (matchedGeometryEffect), month dots are large and tappable with faint per-day entry counts, day opens its page. Remaining: judge morph smoothness on device and polish where it still jumps. | 2–3 |
