// Vellum — the journal that is just a journal.
...
struct VellumApp: App {
    ...
    private static func makeContainer(icloud: Bool) -> ModelContainer {
        let config = ModelConfiguration(
            cloudKitDatabase: icloud ? .private("iCloud.com.wendellbarton.vellum") : .none
        )
        do {
            ...
            do {
                ...
                fatalError("Vellum could not open its store: \(error)")
            }
        }
    }
