// Vellum — the three type registers, ported from the token sheet.
...
struct TypeDisplay: ViewModifier {          // 32 / 1.1 / 500 / -0.01em
    func body(content: Content) -> some View {
        content
            .font(.custom(VellumFont.heading, size: 32).weight(.medium))
            .tracking(32 * -0.01)
            .lineSpacing(spacing(family: VellumFont.heading, size: 32, lineHeight: 1.1))
...
struct TypeTitle: ViewModifier {            // 22 / 1.2 / 500 / -0.005em
    func body(content: Content) -> some View {
        content
            .font(.custom(VellumFont.heading, size: 22).weight(.medium))
            .tracking(22 * -0.005)
            .lineSpacing(spacing(family: VellumFont.heading, size: 22, lineHeight: 1.2))
            .foregroundStyle(Tokens.Text.heading)
    }
}
...
struct TypeWritten: ViewModifier {          // 17 / 1.8 — the writing register
    ...
    func body(content: Content) -> some View {
        ...
        return content
            .font(.custom(VellumFont.body, size: size))
            .lineSpacing(spacing(family: VellumFont.body, size: size, lineHeight: lh))
...
struct TypeMeta: ViewModifier {             // 11 / 1.4 / 0.14em, uppercase
    ...
    func body(content: Content) -> some View {
        ...
        return content
            .font(.custom(VellumFont.meta, size: size))
            .tracking(size * 0.14)
            .textCase(.uppercase)
