...
// when Settings exists. Copy and sequence per docs/vellum-stage2-plan.md,
        ...
        VStack(spacing: Tokens.Space.md) {
            ...
            Button { choose(.icloud) } label: {
                Text("Back up with iCloud")
                    .font(.custom(VellumFont.heading, size: 17).weight(.medium))
    ...
    var body: some View {
        VStack(spacing: Tokens.Space.md) {
            ...
            Text("A week on me.")
                .typeDisplay()
                .multilineTextAlignment(.center)
            Text("Every page, every reflection, free for seven days. After that, Vellum is $29.99 a year — about the price of one good paper notebook.")
                .typeWritten()
                .multilineTextAlignment(.center)
